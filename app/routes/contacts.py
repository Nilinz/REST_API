from fastapi import APIRouter, FastAPI, HTTPException, Depends, status, Security
...
@router.post("/contacts/", response_model=schemas.Contact, description='No more than 10 requests per minute', dependencies=[Depends(RateLimiter(times=10, seconds=60))], status_code=status.HTTP_201_CREATED)
...
@router.get("/contacts/", response_model=schemas.ContactResponse)
